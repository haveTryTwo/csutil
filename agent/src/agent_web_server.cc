...
//
// Agent Web 服务：独立 agent 模块的 HTTP 入口，仅提供 /api/agent/* + 静态页；
// 图书业务仅作为 Agent 工具（list_books/get_book）经回调调用 Control 服务。
// 设计见 docs/AGENT_MODULE_EXTRACTION.md。
...
#include "agent/src/memory_store.h"
 ...
 * @param args_json 工具参数 JSON 串（list_books 支持 prefix/limit/start_after）
 * @return 结果文本
 */
std::string BookToolQuery(const std::string &name, const std::string &args_json) { /*{{{*/
  ...
  if (!doc.HasParseError() && doc.IsObject()) {
    ...
    if (doc.HasMember("start_after") && doc["start_after"].IsString()) {
      start_after = doc["start_after"].GetString();
    }
  ...
  book_mgr::BookResp resp;
  if (CallControl(book_mgr::MakeListReq(prefix, static_cast<uint32_t>(limit), start_after), &resp) != base::kOk) {
    return "list_books 调用 Control 失败";
  }
  const book_mgr::ListBooksResp &l = resp.list_resp();
  char head[128] = {0};
  snprintf(head, sizeof(head), "本页返回 %d 条", l.books_size());
  std::string out = head;
  out += "：\n";
  ...
  if (!l.next_cursor().empty()) {
    out += "\n（还有更多；下一页请传 start_after=\"";
    out += l.next_cursor();
    out += "\"）\n";
  }
  return out;
...
/**
 * @brief 将工作目录切到模块根目录（可执行文件所在 bin/ 的上级目录）
 *
 * 可执行文件位于 <module_root>/bin/agent_web_server，而 conf/ web/ skills/ data/
 * 均相对模块根。为使程序无论从何处启动（如 agent/bin/agent_web_server）都能定位
 * 这些资源，这里依据 argv[0] 推导可执行文件目录，再切到其上级目录。若 argv[0]
 * 不含路径分隔符（经 PATH 调用无法定位），则保持当前工作目录不变。
 *
 * @param arg0 argv[0]，即可执行文件的调用路径
 */
void ChdirToModuleRoot(const char *arg0) { /*{{{*/
  if (arg0 == NULL) return;
  std::string path(arg0);
  std::string::size_type slash = path.find_last_of('/');
  if (slash == std::string::npos) return;  // 经 PATH 调用，无法据此定位，保持当前 cwd
  std::string bin_dir = path.substr(0, slash);  // 可执行文件所在目录（约定为 <root>/bin）
  if (chdir(bin_dir.c_str()) != 0) return;
  if (chdir("..") != 0) return;  // 切到模块根（bin 的上级），conf/web/skills/data 均相对于此
} /*}}}*/
...
int main(int argc, char *argv[]) { /*{{{*/
  ...
  // 切到模块根（可执行文件所在 bin/ 的上级），使 conf/web/skills/data 相对路径
  // 不依赖调用方 cwd；-s/-u 指定的相对路径亦相对该根解析。
  ChdirToModuleRoot(argv[0]);
  ...
  std::string agent_skills_dir = "./skills";
  ...
  if (user_conf.LoadFile(user_conf_path) == kOk) {
    ...
    user_conf.GetValue("agent_skills_dir", "./skills", &agent_skills_dir);
  ...
  if (agent_ret == kOk) {
    ...
    fprintf(stderr, "agent disabled (init ret:%d)\n", agent_ret);
  }
  ...
  fprintf(stderr, "agent web server listening on :%d -> control %s:%d (workers=%d)\n", port, g_control_ip.c_str(),
