...
#ifndef AGENT_SRC_SESSION_STORE_H_
#define AGENT_SRC_SESSION_STORE_H_
...
/**
 * @brief Agent 会话/消息的持久化存储（复用 KvStore，网关进程内单实例）
 *
 * key 设计（有序，便于 Scan）：
...
class SessionStore { /*{{{*/
 public:
   ...
   * @brief 打开底层 LevelDB
   * @param db_path 数据库目录
   * @return kOk 成功；否则透传 KvStore::Init 的错误码
   */
