...
// 
//             char buf[64] = {0};
//             // const std::string &key = field_desc->is_extension() ? (field_desc->full_name()):(field_desc->name());
...
//                         {/*{{{*/
//                             json->append("\"").append(reflection->GetRepeatedString(msg, field_desc, j)).append("\"");
//                             break;
//                         }/*}}}*/
...
//                     {/*{{{*/
//                         json->append("\"").append(key).append("\":");;
//                         json->append("\"").append(reflection->GetString(msg, field_desc)).append("\"");
//                         break;
//                     }/*}}}*/
//                 case ::google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE:
//                     {/*{{{*/
...
base::Code PBToJsonWithOutExtension(const ::google::protobuf::Message &msg, rapidjson::Value::AllocatorType &alloc, rapidjson::Value *json)
{/*{{{*/
    if (json == NULL) return base::kInvalidParam; 
                ...
                switch (field_desc->cpp_type())
                {
                    ...
                    case ::google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
                        {/*{{{*/
                            ...
                            child_arr.PushBack(rapidjson::Value(pb_enum->number()).Move(), alloc); // Note: use number of enum
                            break;
                        }/*}}}*/
                    case ::google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                        {/*{{{*/
        ...
        {/*{{{*/
            ...
            switch (field_desc->cpp_type())
            {
                case ::google::protobuf::FieldDescriptor::CPPTYPE_INT32:
                    {/*{{{*/
                        int value = reflection->GetInt32(msg, field_desc);
                        json->AddMember(rapidjson::Value(key.c_str(), key.size(), alloc).Move(), rapidjson::Value(value).Move(), alloc);
                        break;
                    }/*}}}*/
                case ::google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
                    {/*{{{*/
                ...
                case ::google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
                    {/*{{{*/
                        const ::google::protobuf::EnumValueDescriptor *pb_enum = reflection->GetEnum(msg, field_desc);
                        if (pb_enum == NULL) return base::kInvalidParam;
                        json->AddMember(rapidjson::Value(key.c_str(), key.size(), alloc).Move(), rapidjson::Value(pb_enum->number()).Move(), alloc); // Note: use number of enum instead of string
                        break;
                    }/*}}}*/
                case ::google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                    {/*{{{*/
                        std::string value = reflection->GetString(msg, field_desc);
                        json->AddMember(rapidjson::Value(key.c_str(), key.size(), alloc).Move(), rapidjson::Value(value.c_str(), value.size(), alloc).Move(), alloc);
                        break;
                    }/*}}}*/
                case ::google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE:
                    {/*{{{*/
