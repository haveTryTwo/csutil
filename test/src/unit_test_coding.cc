...
TEST(EncodeVar32, Test_Normal_UInt) { /*{{{*/
  ...
  uint32_t src = 0x11223344;
  const unsigned char dst[5] = {0xC4, 0xE6, 0x88, 0x89, 0x01};
...
TEST(EncodeVar64, Test_Normal_UInt64) { /*{{{*/
  ...
  uint64_t src = 0x1122334455667788;
  const unsigned char dst[9] = {0x88, 0xef, 0x99, 0xab, 0xc5, 0xe8, 0x8c, 0x91, 0x11};
...
TEST(EncodeVar64, Test_Normal_UInt64s) { /*{{{*/
  ...
  var_map[0x3ffffffffff].append(1, 0xff).append(1, 0xff).append(1, 0xff).append(1, 0xff).append(1, 0xff).append(1,
                                                                                                                0x7f);
  var_map[0x1ffffffffffff]
      .append(1, 0xff)
      .append(1, 0xff)
...
TEST(MemcomparableEncode, Test_Normal_EncodeString) { /*{{{*/
  using namespace base;

  std::string src_datas[] = {"a", "abc", "abcd", "abcdefgh", "abd", "abcdefghij", "abcdefghijklmnop", "b"};

  std::string check_datas[8];
  { /*{{{*/
    check_datas[0].append(1, 'a');
    check_datas[0].append(7, 0x0);
    check_datas[0].append(1, 0x1);

    check_datas[1].append("abc");
    check_datas[1].append(5, 0x0);
    check_datas[1].append(1, 0x3);

    check_datas[2].append("abcd");
    check_datas[2].append(4, 0x0);
    check_datas[2].append(1, 0x4);

    check_datas[3].append("abcdefgh");
    check_datas[3].append(1, 0x8);

    check_datas[4].append("abd");
    check_datas[4].append(5, 0x0);
    check_datas[4].append(1, 0x3);

    check_datas[5].append("abcdefgh");
    check_datas[5].append(1, 0x9);
    check_datas[5].append("ij");
    check_datas[5].append(6, 0x0);
    check_datas[5].append(1, 0x2);

    check_datas[6].append("abcdefgh");
    check_datas[6].append(1, 0x9);
    check_datas[6].append("ijklmnop");
    check_datas[6].append(1, 0x8);

    check_datas[7].append("b");
    check_datas[7].append(7, 0x0);
    check_datas[7].append(1, 0x1);
  } /*}}}*/

  std::string check_sort_datas[8] = {check_datas[0], check_datas[1], check_datas[2], check_datas[3],
                                     check_datas[5], check_datas[6], check_datas[4], check_datas[7]};

  fprintf(stderr, "src data:\n");
  for (size_t i = 0; i < sizeof(src_datas) / sizeof(src_datas[0]); ++i) {
    fprintf(stderr, "%s\n", src_datas[i].c_str());
  }
  fprintf(stderr, "\n");

  fprintf(stderr, "After encoding:\n");
  std::string dst_datas[8];
  for (size_t i = 0; i < sizeof(dst_datas) / sizeof(dst_datas[0]); ++i) {
    Code ret = MemcomparableEncode(src_datas[i], &dst_datas[i]);
    EXPECT_EQ(ret, kOk);

    EXPECT_EQ(true, dst_datas[i] == check_datas[i]);

    std::string print_str;
    ret = UrlEncode(dst_datas[i], &print_str);
    fprintf(stderr, "%s\n", print_str.c_str());
  }
  fprintf(stderr, "\n");

  fprintf(stderr, "After decoding:\n");
  std::string src_datas_decode[8];
  for (size_t i = 0; i < sizeof(dst_datas) / sizeof(dst_datas[0]); ++i) {
    Code ret = MemcomparableDecode(dst_datas[i], &src_datas_decode[i]);
    EXPECT_EQ(ret, kOk);

    EXPECT_EQ(true, src_datas_decode[i] == src_datas[i]);

    std::string print_str;
    ret = UrlEncode(src_datas_decode[i], &print_str);
    fprintf(stderr, "%s\n", print_str.c_str());
  }
  fprintf(stderr, "\n");

  fprintf(stderr, "After memcopareble sorting:\n");
  std::sort(dst_datas, dst_datas + sizeof(dst_datas) / sizeof(dst_datas[0]));
  for (size_t i = 0; i < sizeof(dst_datas) / sizeof(dst_datas[0]); ++i) {
    EXPECT_EQ(true, dst_datas[i] == check_sort_datas[i]);

    std::string print_str;
    Code ret = UrlEncode(dst_datas[i], &print_str);
    fprintf(stderr, "%s\n", print_str.c_str());
  }
} /*}}}*/
