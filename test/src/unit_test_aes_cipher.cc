...
//
//     fprintf(stderr, "defuault data size:%zu, encrypt_data size:%zu\n",
//     default_source_data.size(), encrypt_data.size()); fprintf(stderr, "source:  %s\n",
//     default_source_data.c_str());
//
...
//
//     fprintf(stderr, "defuault data size:%zu, encrypt_data size:%zu\n",
//     default_source_data.size(), encrypt_data.size()); fprintf(stderr, "source:size:%zu,  %s\n",
...
//     *(reinterpret_cast<uint32_t*>(const_cast<char*>((decrypt_data.data()))))); delete aes_256;
//     aes_256 = NULL;
// }/*}}}*/
//
...
//     decrypt_data.c_str()); fprintf(stderr, "exception decrypt: "); for (uint32_t i = 0; i <
//     decrypt_data.size(); ++i)
//     {
//         fprintf(stderr, "%02x", (unsigned char)decrypt_data.data()[i]);
//     }
...
TEST(AESCipher, Test_CTR_Encrpyt_Decrypt_Normal_1_Len_Array_AssignIv) { /*{{{*/
  ...
  // source data
  std::string default_source_data_arr[] = {"bbbbbb", "b", "b", "b", "b", "b", "b"};
  ...
  for (int i = 0; i < (int)(sizeof(default_source_data_arr) / sizeof(default_source_data_arr[0]));
       ++i) {
    ...
    fprintf(stderr, "\ndefuault data size:%zu, encrypt_data size:%zu\n", default_source_data.size(),
            encrypt_data.size());
    fprintf(stderr, "source:size:%zu,  %s\n", default_source_data.size(),
            default_source_data.c_str());
