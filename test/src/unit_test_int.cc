...
TEST(GetInt64, Test_Exception_Overflow) { /*{{{*/
  using namespace base;
...
TEST(GetUInt64, Test_Exception_NoData) { /*{{{*/
  using namespace base;

  std::string str_data = "def";
  uint64_t data = 0;
  base::Code ret = GetUInt64(str_data, &data);
  EXPECT_EQ(kStrtollFailed, ret);
} /*}}}*/

TEST(GetUInt64, Test_Exception_NotAllDigits) { /*{{{*/
  using namespace base;

  std::string str_data = "123def";
  uint64_t data = 0;
  base::Code ret = GetUInt64(str_data, &data);
  EXPECT_EQ(kNotAllDigits, ret);
} /*}}}*/

TEST(GetUInt64, Test_Exception_EmptyData) { /*{{{*/
  using namespace base;
...
TEST(GetDouble, Test_Normal_DoubleData) { /*{{{*/
  using namespace base;

  std::string str_datas[] = {"0",
                             "-0",
                             "+0",
                             "123.456",
                             "-123.456",
  ...
  for (int i = 0; i < (int)(sizeof(str_datas) / sizeof(str_datas[0])); ++i) {
    double data = 0;
    base::Code ret = GetDouble(str_datas[i], &data);
    EXPECT_EQ(kOk, ret);
...
TEST(GetDouble, Test_Exception_DoubleData) { /*{{{*/
  using namespace base;

  std::string str_datas[] = {"", "-", "+", "-1.2a", "1.2a", "+1.2a", "gb", "ab123"};
  base::Code dst_rets[] = {kNoDigits,     kNoDigits,     kNoDigits, kNotAllDigits,
                           kNotAllDigits, kNotAllDigits, kNoDigits, kNoDigits};

  for (int i = 0; i < (int)(sizeof(str_datas) / sizeof(str_datas[0])); ++i) {
    double data = 0;
    base::Code ret = GetDouble(str_datas[i], &data);
    EXPECT_EQ(dst_rets[i], ret);
    if (dst_rets[i] != ret) {
      fprintf(stderr, "source:%s ret:%d, dst_ret:%d\n", str_datas[i].c_str(), ret, dst_rets[i]);
    }
  }
} /*}}}*/
...
TEST(BigMultiply, Test_Press_Check_Small_RightNegtiveNum) { /*{{{*/
  ...
  for (int32_t i = 0; i < 1000; ++i) { /*{{{*/
    ...
    for (int32_t j = 0; j > -1000; --j) {
      ...
      EXPECT_EQ(kOk, ret);
      EXPECT_EQ(real_sum, result);
      if (real_sum != result) fprintf(stderr, "%s:%s\n", real_sum.c_str(), result.c_str());
    }
  } /*}}}*/
