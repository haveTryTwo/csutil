...
Code HttpRespProtoFunc(const char *src_data, int src_data_len, int *real_len) { /*{{{*/
  ...
  if (ret == kOk && strcasecmp(tmp.c_str(), kChunked.c_str()) == 0) {
    ...
    ret = HttpProto::GetChunkedMsg(
        std::string(src_data + upper_header_resp.size(), src_data_len - upper_header_resp.size()), &real_chunked_len,
        NULL);
    if (ret != kOk) return ret;
...
Code HttpProto::GetMessageHeader(const std::string &upper_header, const std::string &msg_key,
...
Code HttpProto::GetChunkedMsg(const std::string &body, uint32_t *real_body_len, std::string *chunked_msg) { /*{{{*/
  ...
  while (true) {
    ...
    if (memcmp(pos + kCRLF.size() + content_len, kCRLF.data(), kCRLF.size()) != 0) return kDataNotEnough;
...
Code HttpProto::EncodeToReq(const std::string &url, const std::string &post_params, std::string *post_stream_data,
...
Code HttpProto::EncodeToReq(const std::string &url, std::string *get_stream_data, std::string *host,
                            uint16_t *port) { /*{{{*/
...
Code HttpProto::DecodeFromReq(const std::string &stream_data) { /*{{{*/
  ...
  std::string upper_header_req;
  ret = ToUpperWithOutValue(std::string(stream_data.data(), end_pos - stream_data.data() + end_str.size()),
                            &upper_header_req);
  if (ret != kOk) return ret;
...
Code HttpProto::DecodeFromResponse(const std::string &stream_data, std::string *user_data) { /*{{{*/
  ...
  switch (ret_code) {
    ...
    case kHttpStatusRedirectKeepVerb:
      ...
      break;
    default:
      fprintf(stderr, "http return code:%d, message:%s\n", ret_code, stream_data.c_str());
  ...
  if (r == kOk && strcasecmp(tmp.c_str(), kChunked.c_str()) == 0) {
    r = HttpProto::GetChunkedMsg(
        std::string(stream_data.data() + upper_header_resp.size(), stream_data.size() - upper_header_resp.size()), NULL,
        user_data);
    if (r != kOk) return r;
  } else {
...
const HttpType &HttpProto::GetHttpType() { /*{{{*/ return http_type_; } /*}}}*/

const std::string &HttpProto::GetProtoVersion() { /*{{{*/ return proto_version_; } /*}}}*/

const std::string &HttpProto::GetHost() { /*{{{*/ return host_; } /*}}}*/

const uint16_t HttpProto::GetPort() { /*{{{*/ return port_; } /*}}}*/

const std::string &HttpProto::GetRelativeUrl() { /*{{{*/ return relative_url_; } /*}}}*/

const std::string &HttpProto::GetGetParams() { /*{{{*/ return get_params_; } /*}}}*/

const std::string &HttpProto::GetPostParams() { /*{{{*/ return post_params_; } /*}}}*/
