...
Code RK(const std::string &haystack, const std::string &needle, int *pos) { /*{{{*/
  ...
  for (int i = 0; i <= (int)(haystack.size() - needle.size()); ++i) {
    ...
    if (hash.find(substr) == hash.end()) {  // NOTE:htt, if not found, then continue
    } else {
      *pos = i;
      return kOk;
    }
...
Code SplitUTF8(const std::string &src, std::deque<std::string> *out) { /*{{{*/
  ...
  for (i = 0; i < (int)src.size();) {
    int len = 1;
    if ((src[i] & 0xf8) == 0xf0) {  // 11110www 10xxxxxx 10yyyyyy 10zzzzzz
      len = 4;
    } else if ((src[i] & 0xf0) == 0xe0) {  // 1110xxxx 10yyyyyy 10zzzzzz
      len = 3;
    } else if ((src[i] & 0xe0) == 0xc0) {  // 110yyyyy 10zzzzzz
      len = 2;
    }

    if (len > 1) {  // It is not ascii code, the existing ascii string can be terminated
      if (index != -1) {
