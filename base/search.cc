...
int main(int argc, char *argv[]) { /*{{{*/
  ...
  for (int i = 0; i < (int)(sizeof(test_arr_odd) / sizeof(test_arr_odd[0])); ++i) {
    Code ret = BinaryDescendSearch(arr_des_odd, sizeof(arr_des_odd) / sizeof(arr_des_odd[0]), test_arr_odd[i], &pos,
                                   CompareNum<int>);
    assert(ret == kOk);
  ...
  for (int i = 0; i < (int)(sizeof(test_arr_even) / sizeof(test_arr_even[0])); ++i) {
    Code ret = BinaryDescendSearch(arr_des_even, sizeof(arr_des_even) / sizeof(arr_des_even[0]), test_arr_even[i], &pos,
                                   CompareNum<int>);
    assert(ret == kOk);
  ...
  for (int i = (int)(sizeof(test_arr_odd) / sizeof(test_arr_odd[0])) - 1; i >= 0; --i) {
    Code ret = BinaryAscendSearch(arr_asc_odd, sizeof(arr_asc_odd) / sizeof(arr_asc_odd[0]), test_arr_odd[i], &pos,
                                  CompareNum<int>);
    assert(ret == kOk);
  ...
  for (int i = (int)(sizeof(test_arr_even) / sizeof(test_arr_even[0])) - 1; i >= 0; --i) {
    Code ret = BinaryAscendSearch(arr_asc_even, sizeof(arr_asc_even) / sizeof(arr_asc_even[0]), test_arr_even[i], &pos,
                                  CompareNum<int>);
    assert(ret == kOk);
  ...
  for (int i = 0; i < (int)(sizeof(test_arr_odd) / sizeof(test_arr_odd[0])); ++i) {
    Code ret = BinaryDescendSearch(vec_des_odd.begin(), vec_des_odd.end(), test_arr_odd[i], &it, CompareNum<int>);
    assert(ret == kOk);
  ...
  for (int i = 0; i < (int)(sizeof(test_arr_even) / sizeof(test_arr_even[0])); ++i) {
    Code ret = BinaryDescendSearch(vec_des_even.begin(), vec_des_even.end(), test_arr_even[i], &it, CompareNum<int>);
    assert(ret == kOk);
    fprintf(stderr, "test_value:%d, value:%d\n", test_arr_even[i], *it);
  }
  ...
  for (int i = (int)(sizeof(test_arr_odd) / sizeof(test_arr_odd[0])) - 1; i >= 0; --i) {
    Code ret = BinaryAscendSearch(deq_asc_odd.begin(), deq_asc_odd.end(), test_arr_odd[i], &deq_it, CompareNum<int>);
    assert(ret == kOk);
  ...
  for (int i = (int)(sizeof(test_arr_even) / sizeof(test_arr_even[0])) - 1; i >= 0; --i) {
    Code ret = BinaryAscendSearch(deq_asc_even.begin(), deq_asc_even.end(), test_arr_even[i], &deq_it, CompareNum<int>);
    assert(ret == kOk);
