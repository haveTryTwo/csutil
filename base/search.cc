...
int main(int argc, char *argv[]) { /*{{{*/
  ...
  Print(arr_des_even, arr_des_even + sizeof(arr_des_even) / sizeof(arr_des_even[0]),
        arr_des_even[0]);
  ...
  Print(arr_asc_even, arr_asc_even + sizeof(arr_asc_even) / sizeof(arr_asc_even[0]),
        arr_asc_even[0]);
  ...
  std::vector<int> vec_des_odd(arr_des_odd,
                               arr_des_odd + sizeof(arr_des_odd) / sizeof(arr_des_odd[0]));
  Print(vec_des_odd.begin(), vec_des_odd.end(), vec_des_odd[0]);
  std::vector<int>::iterator it;
  for (int i = 0; i < (int)(sizeof(test_arr_odd) / sizeof(test_arr_odd[0])); ++i) {
    Code ret = BinaryDescendSearch(vec_des_odd.begin(), vec_des_odd.end(), test_arr_odd[i], &it,
                                   CompareNum<int>);
    assert(ret == kOk);
  ...
  std::vector<int> vec_des_even(arr_des_even,
                                arr_des_even + sizeof(arr_des_even) / sizeof(arr_des_even[0]));
  Print(vec_des_even.begin(), vec_des_even.end(), vec_des_even[0]);
  for (int i = 0; i < (int)(sizeof(test_arr_even) / sizeof(test_arr_even[0])); ++i) {
    Code ret = BinaryDescendSearch(vec_des_even.begin(), vec_des_even.end(), test_arr_even[i], &it,
                                   CompareNum<int>);
    assert(ret == kOk);
    fprintf(stderr, "test_value:%d, value:%d\n", test_arr_even[i], *it);
  }
  ...
  std::deque<int> deq_asc_odd(arr_asc_odd,
                              arr_asc_odd + sizeof(arr_asc_odd) / sizeof(arr_asc_odd[0]));
  Print(deq_asc_odd.begin(), deq_asc_odd.end(), deq_asc_odd[0]);
  std::deque<int>::iterator deq_it;
  for (int i = (int)(sizeof(test_arr_odd) / sizeof(test_arr_odd[0])) - 1; i >= 0; --i) {
    Code ret = BinaryAscendSearch(deq_asc_odd.begin(), deq_asc_odd.end(), test_arr_odd[i], &deq_it,
                                  CompareNum<int>);
    assert(ret == kOk);
  ...
  std::deque<int> deq_asc_even(arr_asc_even,
                               arr_asc_even + sizeof(arr_asc_even) / sizeof(arr_asc_even[0]));
  Print(deq_asc_even.begin(), deq_asc_even.end(), deq_asc_even[0]);
  for (int i = (int)(sizeof(test_arr_even) / sizeof(test_arr_even[0])) - 1; i >= 0; --i) {
    Code ret = BinaryAscendSearch(deq_asc_even.begin(), deq_asc_even.end(), test_arr_even[i],
                                  &deq_it, CompareNum<int>);
    assert(ret == kOk);
  ...
  std::pair<std::string, std::string> finds[] = {
      /*{{{*/
      std::pair<std::string, std::string>("aa", ""), std::pair<std::string, std::string>("bb", ""),
      std::pair<std::string, std::string>("cc", ""), std::pair<std::string, std::string>("hh", ""),
      std::pair<std::string, std::string>("ii", ""), std::pair<std::string, std::string>("jj", ""),
      std::pair<std::string, std::string>("oo", ""), std::pair<std::string, std::string>("pp", ""),
      std::pair<std::string, std::string>("qq", ""), std::pair<std::string, std::string>("uu", ""),
      std::pair<std::string, std::string>("vv", ""), std::pair<std::string, std::string>("xx", ""),
  }; /*}}}*/
  fprintf(stderr, "finds num:%zu, single size:%zu\n", sizeof(finds) / sizeof(finds[0]),
          sizeof(finds[0]));
