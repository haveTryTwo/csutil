...
Code AESCipher::Encrypt(const char *source_data, uint32_t len, std::string *encrpyt_data) { /*{{{*/
  ...
  r = EVP_EncryptFinal_ex(&ctx, (unsigned char *)const_cast<char *>(encrpyt_data->data()) + out_len,
                          &out_len);
  if (r != 1) {
...
Code AESCipher::Decrypt(const char *encrypt_data, uint32_t len, std::string *source_data) { /*{{{*/
  ...
  r = EVP_DecryptFinal_ex(&ctx, (unsigned char *)const_cast<char *>(source_data->data()) + out_len,
                          &out_len);
  if (r != 1) {
