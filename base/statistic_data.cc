...
Code DataStatistic::FormatWrite(FILE *fp,
  ...
  for (; it != data_stats.end(); ++it) {
    ...
    fprintf(fp, "|%10s|%14llu|%14llu|%14llu|%14llu|%14llu|\n", it->second.data_type.c_str(),
            (unsigned long long)it->second.total_size, (unsigned long long)it->second.max_size,
            (unsigned long long)it->second.min_size, (unsigned long long)it->second.avg_size,
