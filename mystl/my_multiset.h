...
class my_multiset {
 ...
 public:
  my_multiset() : t(Compare()) {}

  explicit my_multiset(const Compare& comp) : t(comp) {}

  template <class InputIterator>
  my_multiset(InputIterator first, InputIterator last) : t(Compare()) {
    t.insert_equal(first, last);
  }

  template <class InputIterator>
  my_multiset(InputIterator first, InputIterator last, const Compare& comp) : t(comp) {
    t.insert_equal(first, last);
  }

  my_multiset(const my_multiset<Key, Compare, Alloc>& x) : t(x.t) {}

  my_multiset<Key, Compare, Alloc>& operator=(const my_multiset<Key, Compare, Alloc>& x) {
    if (this == &x) {
      return *this;
    }
 ...
 public:
  ...
  void swap(my_multiset<Key, Compare, Alloc>& x) { t.swap(x.t); }

 public:
  typedef my_pair<iterator, bool> pair_iterator_bool;

  my_pair<iterator, bool> insert(const value_type& x) {
    my_pair<typename rep_type::iterator, bool> p = t.insert_equal(x);
