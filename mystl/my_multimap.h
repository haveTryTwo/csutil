...
#ifndef MY_MULTIMAP_H_
#define MY_MULTIMAP_H_
...
class my_multimap {
 ...
 public:
  ...
  my_multimap() : t(Compare()) {}

  explicit my_multimap(const Compare& comp) : t(comp) {}

  template <class InputIterator>
  my_multimap(InputIterator first, InputIterator last) : t(Compare()) {
    t.insert_equal(first, last);
  }

  template <class InputIterator>
  my_multimap(InputIterator first, InputIterator last, const Compare& comp) : t(comp) {
    t.insert_equal(first, last);
  }

  my_multimap(const my_multimap<Key, T, Compare, Alloc>& x) : t(x.t) {}

  my_multimap<Key, T, Compare, Alloc>& operator=(const my_multimap<Key, T, Compare, Alloc>& x) {
    if (this == &x) {
      return *this;
    }
 ...
 public:
  ...
  void swap(my_multimap<Key, T, Compare, Alloc>& x) { t.swap(x.t); }

 public:
  my_pair<iterator, bool> insert(const value_type& x) { return t.insert_equal(x); }
