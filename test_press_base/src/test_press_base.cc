...
{/*{{{*/
    ...
    base::Code ret = strategy::Singleton<test::TestPressController>::Instance()->GetNewPressObject(g_choose_press_name, &press_obj);
    if (ret != base::kOk) 
    {
...
{/*{{{*/
    ...
    fprintf(stderr, "dst_ip_port_protos:%s, thread_num:%d log_interval_ms:%d, choose press name:%s\n",
        test::g_dst_ip_port_protos.c_str(), test::g_thread_num, test::g_log_interval_ms, test::g_choose_press_name.c_str());
